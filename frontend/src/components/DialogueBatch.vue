  ...
  <div class="dialogue-container">
    <el-card class="dialogue-card" shadow="hover">
      ...
      <div class="action-row">
        ...
        >⏹ {{ stopping ? t('dialogue.stopping') : t('dialogue.stopProcessing') }}</el-button>
  ...
  for (let i = 0; i < boxes.value.length; i++) {
    ...
    if (inputMode.value === 'tts') {
      if (box.text.trim() && boxQwen3TtsModeReady(box)) {
        ...
        fd.append(`tts_narrator_id_${i}`, box.ttsNarratorId || '')
...
// status="failed" 区分开）。用专门的错误子类标记"取消"，调用方可以
// 按"用户已取消"展示中性提示，而不是当成真正的处理失败弹错误 Toast。
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (job.status === 'cancelled') {
          ...
          throw new JobCancelledError(job.error || t('dialogue.jobCancelled'))
        }
...
const startProcessing = async () => {
  ...
  try {
    ...
    if (e instanceof JobCancelledError) {
      ElMessage.info(`⏹ ${t('dialogue.jobCancelled')}`)
    } else {
...
// 停止处理：请求后端取消当前批量任务（协作式取消——多数情况下会在
// "处理完当前对话框" 或 "下一个处理阶段" 时才真正生效，只有某个对话框
// 恰好正在跑 MFA 对齐时才能被立即终止；具体说明见后端 app.py/
// pipeline.py 的注释）。前端这边点击后立刻停止轮询并恢复表单可交互，
// 不等待后端确认取消已经落地，避免用户对着停止按钮干等。
const stopProcessing = async () => {
  ...
  try {
    ...
    if (!res.ok && res.status !== 409) {
      throw new Error(data.error || t('dialogue.stopFailed'))
    }
    ElMessage.info(`⏹ ${t('dialogue.stopRequested')}`)
  } catch (e: any) {
    ElMessage.error(`❌ ${e?.message || String(e)}`)
  } finally {
    pollActive = false
