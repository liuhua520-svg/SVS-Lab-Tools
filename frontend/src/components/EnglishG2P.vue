    ...
    <el-form label-width="120px" class="input-section">
      ...
      <div v-if="customArpaMap || customVocaloidMap" class="custom-dict-banner">
        <el-tag v-if="customArpaMap" type="success" closable @close="clearCustomDict('arpa')">
          {{ t('englishG2P.customSynthVLoaded') }}
        </el-tag>
    ...
    <div v-if="results.length > 0" class="results-section">
      <div class="results-header">
        ...
        <div class="result-actions">
          <el-dropdown trigger="click" @command="(cmd: 'arpa' | 'synthv' | 'vocaloid' | 'all') => exportResults('json', cmd)">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                ...
                <el-dropdown-item command="synthv">{{ t('englishG2P.downloadSynthVOnly') }}</el-dropdown-item>
          ...
          <el-dropdown trigger="click" @command="(cmd: 'arpa' | 'synthv' | 'vocaloid' | 'all') => exportResults('csv', cmd)">
            <el-button size="small" type="primary">
              <el-icon><Download /></el-icon>
      ...
      >
        ...
        <el-table-column prop="synthv" :label="t('englishG2P.synthv')" min-width="180">
          <template #default="scope">
            <code class="phoneme-code synthv-code">{{ scope.row.synthv }}</code>
          </template>
        </el-table-column>
...
/**
 * 结果表格共三列音素，均以"音素"（而非"单词"）为键，互不影响：
 *   - ARPAbet 列：始终是后端 g2p_en 的原始结果（小写去重音），任何自定义
 *     词典都不会改变这一列。
 *   - Synthesizer V 列：以 ARPAbet 列为输入，逐音素查 customArpaMap
 *     （synthesizerv notation）替换，未命中的音素原样透传（见 arpaToSynthV）。
 *   - VOCALOID 列：以 ARPAbet 列为输入，逐音素查 customVocaloidMap，未命中
 *     回退到内置 ARPABET_TO_VOCALOID 表（见 arpaToVocaloid）。
 *
 * 用户可通过"导入词典"按钮上传符合词典导入格式的 JSON/CSV 文件：
 *   - JSON: {"notation": "synthesizerv" | "vocaloid", "entries": {PHONEME: mapped}}
 *   - CSV : 两列（首行表头 "word,phonemes" 会被自动跳过），第一列即
 *           ARPABET 音素记号，第二列即该音素对应的目标记号
 * "synthesizerv" → 覆盖 customArpaMap，改变 Synthesizer V 列
 * "vocaloid"     → 覆盖 customVocaloidMap，改变 VOCALOID 列
 *
 * 导入后仅覆盖表中出现的音素键，未出现的音素仍使用各自的默认结果兜底。
 */
...
const handleImportFile = async (event: Event) => {
  ...
  try {
    ...
    if (notation === 'vocaloid') {
      ...
      ElMessage.success(t('englishG2P.customSynthVLoaded'))
    } else {
      throw new Error(t('englishG2P.importNotationError'))
    }

    // 已有结果时，用新导入的映射表重新计算 Synthesizer V / VOCALOID 列
    // （ARPAbet 列始终是原始 g2p_en 结果，不受影响）
    recomputeResults()
  } catch (err) {
...
const recomputeResults = () => {
  ...
  results.value = results.value.map((r) => {
    // ARPAbet 列始终是原始 g2p_en 结果（formatArpaDisplay 处理后），
    // 不受任何自定义词典影响；VOCALOID 列基于它独立计算；
    // Synthesizer V 列基于它单独应用 customArpaMap。
    const arpa = r.rawArpa
    return {
      ...r,
      arpa,
      vocaloid: arpaToVocaloid(arpa),
      synthv: arpaToSynthV(arpa),
    }
  })
}
...
/** 将一个 ARPABET 音素字符串（formatArpaDisplay 处理后的小写、去重音形式，
 *  如 "hh ah l ow"）转换为 Synthesizer V 音素字符串：按空格拆分为逐个
 *  音素 token，逐个查自定义 synthesizerv 词典替换，未命中的音素保持原样
 *  透传（即等于 ARPAbet 值）。
 *  词典键是"音素"而非"单词"（如 {"aa": "e i", "hh": "x"}），必须和
 *  arpaToVocaloid 一样逐音素查找，不能按整词查
 *  customArpaMap.value[r.word]（那样永远查不到，词典形同虚设）。
 *  仅在加载了自定义 synthesizerv 词典时才会与 ARPAbet 列不同；未加载时
 *  该列与 ARPAbet 列完全一致。不影响 ARPAbet 列本身，也不影响
 *  arpaToVocaloid（VOCALOID 列始终基于原始 ARPAbet 独立计算）。 */
const arpaToSynthV = (displayArpa: string): string => {
  if (!displayArpa) return ''
  if (!customArpaMap.value) return displayArpa
  return displayArpa
    .split(/\s+/)
    .filter(Boolean)
    .map((ph) => {
      const clean = ph.toLowerCase().replace(/\d+$/, '')
      return customArpaMap.value && clean in customArpaMap.value
        ? customArpaMap.value[clean]
        : ph
    })
    .join(' ')
}
...
const extractAndConvert = async () => {
  ...
  try {
    ...
    results.value = data.results.map((r: any, idx: number) => {
      ...
      // ARPAbet 列始终是后端 g2p_en 的原始结果，不受任何自定义词典影响。
      // VOCALOID 列和 Synthesizer V 列都基于这个原始 ARPAbet 独立计算，
      // 互不影响：VOCALOID 查 customVocaloidMap，Synthesizer V 查
      // customArpaMap（synthesizerv notation，逐音素替换表，详见
      // arpaToSynthV）。
...
/**
 * 导出结果为文件。VOCALOID 与 Synthesizer V 两个 scope 导出的文件符合
 * 词典导入格式，可直接re-import（与 dictionary_manager.py 的
 * _extract_entries_from_json_payload / import_csv_text 保持一致）：
 *   - JSON: {"notation": "synthesizerv" | "vocaloid", "entries": {WORD: phones}}
 *   - CSV : "word,phonemes" 两列（首行表头）
 * ARPAbet scope 导出的是原始 g2p_en 结果的单词对照表（供查阅/存档），
 * 不带 notation 字段，因为 ARPAbet 列本身不接受"单词→音素"格式的自定义
 * 词典覆盖（自定义 synthesizerv 词典是按音素替换，见 arpaToSynthV）。
 *
 * scope 参数决定实际下载哪些文件：
 *   'arpa'     → 仅下载 ARPAbet 单词对照表
 *   'synthv'   → 仅下载 Synthesizer V 词典
 *   'vocaloid' → 仅下载 VOCALOID 词典
 *   'all'      → 三个文件都下载（默认）
 */
const exportResults = (format: 'json' | 'csv', scope: 'arpa' | 'synthv' | 'vocaloid' | 'all' = 'all') => {
  if (results.value.length === 0) return

  // 去重后的词条（以最新出现的单词/音素为准，词典本身按单词唯一存储）
  const arpaEntries: Record<string, string> = {}
  const synthvEntries: Record<string, string> = {}
  const vocaloidEntries: Record<string, string> = {}
  for (const r of results.value) {
    if (!r.word) continue
    arpaEntries[r.word] = r.arpa
    synthvEntries[r.word] = r.synthv
    vocaloidEntries[r.word] = r.vocaloid
  }

  const wantArpa = scope === 'arpa' || scope === 'all'
  const wantSynthV = scope === 'synthv' || scope === 'all'
  const wantVocaloid = scope === 'vocaloid' || scope === 'all'

  if (format === 'json') {
    if (wantArpa) {
      downloadFile(
        JSON.stringify({ entries: arpaEntries }, null, 2),
        'english-g2p-arpabet.json',
        'application/json',
      )
    }
    if (wantSynthV) {
      downloadFile(
        JSON.stringify({ notation: 'synthesizerv', entries: synthvEntries }, null, 2),
    ...
    if (wantArpa) {
      downloadFile('\ufeff' + toCsv(arpaEntries), 'english-g2p-arpabet.csv', 'text/csv')
    }
    if (wantSynthV) {
      downloadFile('\ufeff' + toCsv(synthvEntries), 'english-g2p-synthesizerv.csv', 'text/csv')
    }
    if (wantVocaloid) {
