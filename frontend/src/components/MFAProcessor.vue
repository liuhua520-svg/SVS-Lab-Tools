<template>
  <div class="processor-container">
    <el-card class="processor-card" shadow="hover">
      ...
      <el-form :model="formData" :disabled="processing" label-position="top" class="processor-form">
        ...
        <template v-if="inputMode === 'tts'">
          ...
          <el-form-item :label="t('processor.narrator')">
            ...
            <el-button link type="primary" @click="openNarratorManager" style="margin-left: 8px" :disabled="processing">
          ...
          <!-- VoiceDesign 专用：Emotion（拼接进声音描述）+ Seed（随机种子，
               便于复现同一次生成结果）。仅 voice_design 模式显示——
               custom_voice/voice_clone 均不使用这两个参数。
               Emotion 不限定于固定的几个选项（模型支持的情绪词远不止
               Happy/Sad 等），用 filterable + allow-create 的下拉框：
               常见情绪可以直接选，也可以输入任意自定义描述文本回车确认。 -->
          <el-form-item v-if="ttsConfig.engine === 'qwen3_tts' && qwen3TtsMode === 'voice_design'" :label="t('processor.qwen3TtsEmotion')">
            <el-select
              v-model="qwen3TtsEmotion"
              filterable
              allow-create
            ...
            <span style="margin-left: 10px; font-size: 12px; color: var(--el-text-color-secondary)">
              {{ t('processor.qwen3TtsEmotionHint') }}
            </span>
          </el-form-item>
          <el-form-item v-if="ttsConfig.engine === 'qwen3_tts' && qwen3TtsMode === 'voice_design'" :label="t('processor.qwen3TtsSeed')">
            <el-input-number v-model="qwen3TtsSeed" :min="0" :step="1" controls-position="right" style="width: 200px" />
            <el-button style="margin-left: 10px" @click="randomizeQwen3TtsSeed">
              🎲 {{ t('processor.qwen3TtsSeedRandomize') }}
            </el-button>
            <span style="margin-left: 10px; font-size: 12px; color: var(--el-text-color-secondary)">
              {{ t('processor.qwen3TtsSeedHint') }}
            </span>
          </el-form-item>
            ...
            <div style="width: 100%">
              <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap">
                <el-button
                  ...
                  :disabled="!ttsSegmentPreviewReady || processing"
                >
        ...
        <el-dialog v-model="textOptimizer.visible" :title="t('processor.textOptimize')" width="640px" :close-on-click-modal="!processing" :show-close="!processing">
          ...
          <div class="text-optimize-toolbar">
            ...
            <el-button size="small" :disabled="processing" :loading="textOptimizer.loading === 'traditional_to_simplified'" @click="runTextOptimize('traditional_to_simplified')">
              🇨🇳 {{ t('processor.textOptimizeTraditionalToSimplified') }}
            </el-button>
		...
		>
		  ...
		  <div style="margin-top: 6px">
			<el-button size="small" :disabled="processing" @click="openTextOptimizer(formData, 'text')">
			  🛠️ {{ t('processor.textOptimize') }}
			</el-button>
			<el-button size="small" :disabled="processing" @click="openFindReplace(formData, 'text')">
        ...
        <el-form-item style="margin-top: 20px">
          ...
          >
            ⏹ {{ stopping ? t('processor.stopping') : t('processor.stopProcessing') }}
          </el-button>
...
type TtsNarrator = {
  ...
  // VoiceDesign 专用：自由文本情绪描述 + 随机种子，与主面板的
  // qwen3TtsEmotion/qwen3TtsSeed 同款语义（见下方 buildQwen3TtsOptionsForPreview
  // 及各处 box/表单同名字段的注释），随预设一起保存/套用。
...
// VoiceDesign 专用：Emotion（''表示"无"，不拼接）+ Seed（随机种子，null
// 表示不传，由后端保持每次随机）。QWEN3_TTS_EMOTION_SUGGESTIONS 只是下拉框
// 里预置的常见选项，供快速选择——Emotion 实际上是拼接进声音描述的自由
// 文本，模型能理解的情绪词远不止这几个，因此下拉框用 filterable +
// allow-create，用户可以直接输入任意自定义情绪/语气描述。
const QWEN3_TTS_EMOTION_SUGGESTIONS = ['Happy', 'Sad', 'Angry', 'Fear', 'Disgust', 'Surprise', 'Calm']
const qwen3TtsEmotion = ref('')
const qwen3TtsSeed = ref<number | null>(null)
// Seed 输入框右边的"随机"按钮：填一个 [0, 2^31 - 1] 范围内的随机整数，
// 上限对齐 32 位有符号整数（torch.manual_seed 接受的范围远大于此，但
// 常见"种子"输入习惯都用这个区间，足够覆盖"每次点一下换一个种子"的
// 使用场景）。
const randomSeedValue = () => Math.floor(Math.random() * 2147483648)
const randomizeQwen3TtsSeed = () => { qwen3TtsSeed.value = randomSeedValue() }
...
// 语音预设管理弹窗内 Seed 字段专用的"随机"按钮：与主面板的
// randomizeQwen3TtsSeed 同一套随机范围，只是写入目标不同
// （narratorForm.value.qwen3_tts_seed，而不是主面板的 qwen3TtsSeed）。
...
const handleNarratorSelect = (narratorId: string) => {
  if (!narratorId) {
    ...
    qwen3TtsEmotion.value = ''
    qwen3TtsSeed.value = null
    qwen3TtsRefAudioFile.value = null
    qwen3TtsRefAudioPath.value = ''
    return
  }
  ...
  if (n.engine === 'qwen3_tts') {
    ...
    qwen3TtsEmotion.value = n.qwen3_tts_emotion || ''
    qwen3TtsSeed.value = n.qwen3_tts_seed ?? null
...
const buildQwen3TtsOptionsForPreview = async (): Promise<Record<string, any> | undefined> => {
  ...
  if (qwen3TtsMode.value === 'custom_voice') {
    ...
    if (qwen3TtsEmotion.value) opts.emotion = qwen3TtsEmotion.value
    if (qwen3TtsSeed.value !== null && qwen3TtsSeed.value !== undefined) opts.seed = qwen3TtsSeed.value
...
// 用户主动点击"停止"后，任务最终会被后端标记为 status="cancelled"（与
// status="failed" 区分开）。这里用一个专门的错误子类标记"取消"，
// 调用方 catch 到之后可以按"用户已取消"展示中性提示，而不是当成
// 真正的处理失败弹错误 Toast。
    ...
    const tick = async () => {
      try {
        ...
        if (job.status === 'done') {
          ...
          throw new JobCancelledError(job.error || t('processor.jobCancelled'))
        } else if (job.status === 'failed') {
          throw new Error(job.error || t('processor.jobFailed'))
        }
        // queued / running: 继续轮询
...
// 停止处理：请求后端取消当前任务（协作式取消，具体生效时机见后端
// app.py/pipeline.py 里的说明——多数阶段会在下一个阶段边界才真正停下，
// 只有 MFA 对齐阶段支持立即终止子进程）。前端这边不等待任务真正停下，
// 点击后立刻停止轮询并恢复表单可交互，避免用户对着一个"看起来卡住"的
// 停止按钮干等；后端会在后台继续把这次运行标记为 cancelled 并做清理。
const stopProcessing = async () => {
  if (!currentJobId.value || stopping.value) return
  stopping.value = true
  try {
    const res = await fetch(`/api/pipeline/job/${currentJobId.value}/cancel`, { method: 'POST' })
    const data = await res.json().catch(() => ({}))
    if (!res.ok && res.status !== 409) {
      throw new Error(data.error || t('processor.stopFailed'))
    }
    ElMessage.info(`⏹ ${t('processor.stopRequested')}`)
  } catch (e: any) {
    ElMessage.error(`❌ ${e?.message || String(e)}`)
  } finally {
    // 无论后端确认与否，前端都不再继续等待这次运行——停止轮询、恢复表单，
    // 让用户可以立刻修改参数或重新开始，不必等取消真正落地。
    clearJobPolling()
...
// 统一处理 waitForJobFinished 抛出的错误：用户主动点击"停止"产生的
// JobCancelledError 按中性提示展示（不算处理失败，不写入 error.value
// 触发页面上的错误态展示），其它异常仍按原来的失败流程处理。
const handleJobError = (e: any) => {
  if (e instanceof JobCancelledError) {
    ElMessage.info(`⏹ ${t('processor.jobCancelled')}`)
    return
  }
    ...
    try {
      ...
      if (ttsConfig.value.engine === 'qwen3_tts') {
        ...
        if (qwen3TtsMode.value === 'custom_voice') {
          ...
          if (qwen3TtsEmotion.value) qwen3OptionsForSubmit.emotion = qwen3TtsEmotion.value
          if (qwen3TtsSeed.value !== null && qwen3TtsSeed.value !== undefined) qwen3OptionsForSubmit.seed = qwen3TtsSeed.value
        } else if (qwen3TtsMode.value === 'voice_clone') {
